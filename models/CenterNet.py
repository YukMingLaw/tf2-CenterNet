from tensorflow.keras.layers import Input, Conv2DTranspose, BatchNormalization, ReLU, Conv2D, Lambda, Dropout,Add
...
def centernet(num_classes, backbone='efficientnet',
              input_size=512,
              max_objects=100,
              score_threshold=0.1,
              nms=False,
              flip_test=False,
              freeze_bn=True):
    assert backbone in ['resnet50','mobilenetv2','efficientnet']
    ...
    if backbone == 'resnet50':
        BackboneNet = tf.keras.applications.ResNet50(input_tensor=image_input,include_top=False,weights=None)
    elif backbone == 'mobilenetv2':
        BackboneNet = tf.keras.applications.MobileNetV2(input_tensor=image_input, include_top=False, weights=None)
    elif backbone == 'efficientnet':
        BackboneNet = EfficientNet('efficientnet-b2', image_input)

    # Apply FPN
    # (b, 64, 64, 2048)
    feature_s8 = BackboneNet.get_layer('add_4').output
    feature_s8 = Conv2D(128,(1, 1),strides=1,use_bias=False,padding='same',kernel_regularizer=l2(5e-4))(feature_s8)
    feature_s8 = BatchNormalization()(feature_s8)
    feature_s8 = ReLU()(feature_s8)
    # (b, 32, 32, 2048)
    feature_s16 = BackboneNet.get_layer('add_10').output
    feature_s16 = Conv2D(256, (1, 1), strides=1, use_bias=False, padding='same', kernel_regularizer=l2(5e-4))(feature_s16)
    feature_s16 = BatchNormalization()(feature_s16)
    feature_s16 = ReLU()(feature_s16)
    # (b, 16, 16, 2048)
    feature_s32 = BackboneNet.outputs[-1]
    # decoder
    x = Conv2DTranspose(256, (4, 4), strides=2, use_bias=False, padding='same',kernel_initializer='he_normal',kernel_regularizer=l2(5e-4))(feature_s32)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Add()([x,feature_s16])
    x = Conv2DTranspose(128, (4, 4), strides=2, use_bias=False, padding='same', kernel_initializer='he_normal',kernel_regularizer=l2(5e-4))(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Add()([x, feature_s8])
    x = Conv2DTranspose(64, (4, 4), strides=2, use_bias=False, padding='same', kernel_initializer='he_normal',kernel_regularizer=l2(5e-4))(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
