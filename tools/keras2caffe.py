...
def set_padding(config_keras, input_shape, config_caffe):
    if config_keras['padding'] == 'valid':
        ...
        pad_w = int(math.ceil((kernel_size[1] * out_w - (kernel_size[1] - strides[1]) * (out_w - 1) - w) / 2))

        out_h = math.ceil(h / float(strides[0]))
        pad_h = int(math.ceil((kernel_size[0] * out_h - (kernel_size[0] - strides[0]) * (out_h - 1) - h) / 2))
...
def convert(keras_model, caffe_net_file, caffe_params_file):
    ...
    for layer in keras_model.layers:
        ...
        # print(name,layer_type,type(layer.output).__name__)
        ...
        if layer_type == 'Conv2D' or layer_type == 'Convolution2D':
            ...
            if 'my_swish' in bottom:
                outputs_name = outputs[bottom] + '_mul'
                caffe_net[name] = L.Convolution(caffe_net[outputs_name], **kwargs)
            else:
            ...
            caffe_net[name] = L.BatchNorm(caffe_net[outputs[bottom]], in_place=True,batch_norm_param={'use_global_stats': True})
            ...
            caffe_net[name_s] = L.Scale(caffe_net[name], in_place=True,scale_param={'bias_term': config['center']})
            ...
            if config['use_bias']:
                ...
                if type(layer._inbound_nodes[0].inbound_layers[0]).__name__ == 'Flatten':
                    ...
                    for i in range(weight.shape[0]):
                        weight[i] = np.array(
                            weight[i].reshape(flatten_shape[1], flatten_shape[2], flatten_shape[3]).transpose(2, 0,
                                                                                                              1).reshape(
                                weight.shape[1]))
            ...
            caffe_net[name] = L.ReLU(caffe_net[outputs[bottom]], negative_slope = 0., in_place=True)
        ...
        # TODO
            ...
            #                             stride=1, pad_h=padding[0][0],
            #                             pad_w=padding[1][0], pool=P.Pooling.MAX)
            ...
            caffe_net[name] = L.Deconvolution(caffe_net[outputs[bottom]], convolution_param=dict(**kwargs))
            ...
            if config['activation'] == 'relu':
                ...
                raise Exception('Unsupported activation ' + config['activation'])
        elif layer_type == 'MySwish':
            caffe_net[name] = L.Sigmoid(caffe_net[outputs[bottom]])
            name_mul = name + '_mul'
            layers = []
            layers.append(caffe_net[name])
            layers.append(caffe_net[outputs[bottom]])
            caffe_net[name_mul] = L.Eltwise(*layers,eltwise_param=dict({'operation':0}))
        else:
...
class MySwish(tf.keras.layers.Layer):
    def __init__(self,**kwargs):
        super(MySwish, self).__init__(**kwargs)

    def call(self, input):
        return tf.nn.swish(input)

model = tf.keras.models.load_model('../save/efficient_fpn_debug.h5',custom_objects={'MySwish': MySwish()})

convert(model, 'efficientnet.prototxt', 'efficientnet.caffemodel')
